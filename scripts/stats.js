...
class StatsModule {
    ...
    // ── 4. Auditorías PGA ─────────────────────────────────────
    renderPgaTable() {
        const container = document.getElementById('statsPgaTable');
        if (!container) return;

        const pgaAudits = this.data
            .filter(a => a.pga === true)
            .sort((a, b) => (b.audit_date || '').localeCompare(a.audit_date || ''));

        if (pgaAudits.length === 0) {
            container.innerHTML = `
                <div class="no-repeated">
                    <i class="fas fa-trophy" style="color:#2563eb;font-size:1.5rem;"></i>
                    <span>No hay auditorías PGA en el período seleccionado.</span>
                </div>`;
            return;
        }

        let html = `
            <div class="pga-summary">
                <span class="pga-badge">${pgaAudits.length} registro(s) PGA</span>
            </div>
            <table class="stats-table">
                <thead>
                    <tr>
        ...
        this.data.forEach(a => {
            ...
            if (a.errors_found) {
                ...
                this.ERROR_FIELDS.forEach(f => {
                    const val = parseInt(a[f]) || 0;
                    if (val > 0) byCell[c].errorTypes[f] += val;
                });
            } else {
                byCell[c].ok++;
            }
                    ...
                    tooltip: {
                        callbacks: {
                            afterBody: ctx => {
                                ...
                                const errors = Object.entries(byCell[cell].errorTypes)
                                    .filter(([_, v]) => v > 0)
                                    .sort((a, b) => b[1] - a[1])
                                    .slice(0, 3)
                                    .map(([f, v]) => this.ERROR_LABELS[f] + ': ' + v)
                                    .join(', ');
                                const res = ['Total: ' + total, '% Error: ' + pct + '%'];
                                if (errors) res.push('Top Errores: ' + errors);
                                return res;
                            }
                        }
                    }
